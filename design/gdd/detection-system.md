...
| Seeker AI State Machine | Writes to | `DetectionOutput { float SuspicionLevel, SeekState RequestedState, Vector3 LastKnownPlayerPosition, bool IsLoSActive }` each FixedUpdate | Detection System owns suspicion, state transitions, and catch dwell check; Seeker AI owns navigation and animation responses. `IsLoSActive` is true when LoS raycast succeeded this tick; used by Seeker AI to track `timeSinceLastDirectLoS` for the Chase lost-player gate (F-S4 in Seeker AI GDD). |
